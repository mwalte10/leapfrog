...
The simulation model is implemented in a header-only C++ library located in [`inst/include/leapfrog.h`](inst/include/leapfrog.h). This location allows the C++ code to be imported in other R packages via specifying `LinkingTo: leapfrog` in the `DESCRIPTION` file.
